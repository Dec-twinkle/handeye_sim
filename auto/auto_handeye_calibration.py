...
class auto_handeye_calibration(object):
    ...
    def camera_pose_simple(self,verbose=0):
        ...
        if verbose == 1:
            ...
            plt.show()
        init_base_camepose = getBaseCampose(self.Hobj2camera[:3,:3])
        ...
        for i in range(angle.shape[0]):
            R = transforms3d.euler.euler2mat(ax+angle[i, 0], ay+angle[i, 1], az+angle[i, 2])
        ...
        if self.next_step_method == 1 or self.next_step_method == 3  :
            ...
            for i in range(len(expect_camera_list)):
                ...
                if self.next_step_method==3:
                    ...
                    score+=no_local_score
                sco_list.append([i, score, robot_pose])
            sco_list.sort(key=lambda x: x[1])
            return sco_list[:][2].reverse()
            ...
            for i in range(len(expect_camera_list)):
                ...
                no_local_score = self.score_no_local(robot_pose)
                sco_list.append([i, no_local_score, robot_pose])
            sco_list.sort(key=lambda x: x[1])
            return sco_list[:][2].reverse()
        else:
            robot_pose_list = []
            ...
            return random.shuffle[robot_pose_list]
    ...
    def run(self):
        ...
        while(len(self.image)<self.picture_number):
            robot_list = self.score_main(simple_campose)
            for pose in robot_list:
